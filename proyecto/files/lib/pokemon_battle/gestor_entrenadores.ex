defmodule PokemonBattle.GestorEntrenadores do
  ...
  def handle_call({:get_entrenador, pid}, _from, state) do
    case Map.get(state.sesiones, pid) do
      nil -> {:reply, {:error, "No estas en sesion"}, state}
      usuario ->
  ...
  def handle_cast({:intercambio, usuario1, id_pk1, usuario2, id_pk2}, state) do
    e1 = Enum.find(state.entrenadores, &(&1.usuario == usuario1))
    e2 = Enum.find(state.entrenadores, &(&1.usuario == usuario2))

    pk1 = Enum.find(e1.inventario, &(&1.id == id_pk1))
    pk2 = Enum.find(e2.inventario, &(&1.id == id_pk2))

    e1_nuevo = %{e1 | inventario: Enum.reject(e1.inventario, &(&1.id == id_pk1)) ++ [pk2]}
    e2_nuevo = %{e2 | inventario: Enum.reject(e2.inventario, &(&1.id == id_pk2)) ++ [pk1]}

    entrenadores = Enum.map(state.entrenadores, fn e ->
      cond do
        e.usuario == usuario1 -> e1_nuevo
        e.usuario == usuario2 -> e2_nuevo
        true -> e
      end
    end)
